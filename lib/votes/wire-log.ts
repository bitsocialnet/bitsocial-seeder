...
const bundleCidForBytes = async (bytes: Uint8Array) => CID.createV1(dagCbor.code, await sha256.digest(bytes)).toString()

// EIP-55 checksummed when it's a real 20-byte address, so log lines match what wallets
// display verbatim; raw hex fallback for anything else.
const hex = (bytes: Uint8Array) => {
  const plain = `0x${Array.from(bytes, (b) => b.toString(16).padStart(2, '0')).join('')}`
  try {
    return getAddress(plain)
  }
  catch {
    return plain
  }
...
const describeBundleVotes = (bundle: Uint8Array) => {
  try {
    const wire: any = dagCbor.decode(bundle)
    const votes = (wire?.votes ?? []).map((vote: any) => {
...
export const parseGossipMessage = (bytes: Uint8Array): {kind: string, record?: any, bundle?: any} => {
  try {
    const message: any = dagCbor.decode(bytes)
...
export const describeRootHeartbeat = (record: any) => describeRecord(record)

export const describeLiveBundle = async (bundle: Uint8Array) =>
