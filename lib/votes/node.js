...
export const createVotesNode = async ({votesConfig, log = console.log}) => {
  ...
  const libp2p = await createLibp2p({
    ...
    // interface addrs; behind NAT the public address is learned the same way the daemon's
    // Kubo learns it: identify observed-addresses from the bootstrap connections, confirmed
    // by AutoNAT dial-backs — which is also what AutoTLS waits for.
    addresses: {listen},
