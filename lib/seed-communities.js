...
const handleCommunityUpdate = (community, communityKey) => {
  ...
  try {
    ...
    for (const {cid, name, pubsubTopic} of pubsubRoutingPins) {
      ...
      const shouldQueue = Date.now() - lastQueuedAtMs >= provideIntervalMs

      if (shouldQueue) {
        pubsubRoutingQueue.enqueueTx(tx, {communityKey, cid, name, pubsubTopic, communityAddress: community.address})
        tx.execute(
...
export const providePubsubTopicRoutingCids = async () => {
  for (const [communityKey, community] of Object.entries(communitiesUpdating)) {
    ...
    const tx = db.transaction()
    try {
      for (const {cid, name, pubsubTopic} of getCommunityPubsubTopicRoutingPins(community)) {
        const throttleRow = tx.query(
          'SELECT last_queued_at FROM pubsub_routing_provides WHERE community_key = ? AND cid = ?',
          [communityKey, cid]
        )[0]
        const lastQueuedAtMs = (throttleRow?.last_queued_at || 0) * 1000
        if (Date.now() - lastQueuedAtMs < provideIntervalMs) {
          continue
        }
        pubsubRoutingQueue.enqueueTx(tx, {communityKey, cid, name, pubsubTopic, communityAddress: community.address})
        tx.execute(
